...
public class CarLayer {
    ...
    private final String TYPE_BUS = "bus";
    private final String TYPE_TRUCK = "truck";
    private final String TYPE_TRAILER = "trailer";
    ...
    private Image getImageForVehicle(String vehicleId) {
        ...
        // Nếu là xe buýt
        if (vclass.contains(TYPE_BUS) || type.contains(TYPE_BUS)) {
            if (busImage != null) return busImage;
            // Nếu không có hình bus thì dùng hình xe con
            return carImage;
        }
        // Nếu là xe tải hoặc xe kéo
        if (vclass.contains(TYPE_TRUCK) || vclass.contains(TYPE_TRAILER) || type.contains(TYPE_TRUCK) || type.contains(TYPE_TRAILER)) {
