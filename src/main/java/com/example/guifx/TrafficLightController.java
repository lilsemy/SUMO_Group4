...
public class TrafficLightController {
  ...
  public TrafficLightController() {
    ...
    for (String id : tlIds) {
      TrafficLightModel tl =
          new TrafficLightModel(
              id, TrafficLight.getPhase(id), TrafficLight.getRedYellowGreenState(id));
  ...
  public TraCIPosition getTlPosition(TrafficLightModel tl) {
    ...
    // tlController.getTlPosition(tlController.getTlList().get("n2")); --> Possible Calling Method
    // e.g. from TrafficLightLayer (it needs the TrafficLightController)
  }
}
