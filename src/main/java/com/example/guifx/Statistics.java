...
public class Statistics {
  ...
  private Map<String, Integer> vehicleCountsPerEdge =
      new HashMap<>(); // Vehicles per Edge (density)

  // Traffic Light Data Structure
  private Map<String, Integer> currentTrafficLightStates =
      new HashMap<>(); // Count of Traffic lights per phase
  ...
  public void updateVehicles(double currentTime) {
    this.currentVehicles =
        simCon
            ...
            .getVehiclesMap(); // getVehicles() still needs to be implemented
  ...
  /**
   * Fetches the current state of all traffic lights from the simulation and counts how many are
   * currently red, yellow, or green
   */
  ...
  public void printAllStatistics(double currentTime) {
    ...
    System.out.println(
        "Travel times (for vehicles that have left the simulation): "
            + calculateTravelTimes(currentTime));
