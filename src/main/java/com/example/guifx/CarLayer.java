...
public class CarLayer {
  ...
  CarLayer(Pane carLayerPane) {
    ...
    // Nếu carLayerPane là null, xóa hết các node con (phòng trường hợp lỗi, nhưng thường không xảy
    // ra)
  ...
  private Image getImageForVehicle(String vehicleId) {
    ...
    if (vclass.contains("truck")
        || vclass.contains("trailer")
        || type.contains("truck")
  ...
  public void updateCars() {
    ...
    for (String carId : newCarIds) {
      ...
      if (carView != null) {
        ...
        Point2D javaPosition =
            MapUtil.worldToScreen(new Point2D(sumoPosition.getX(), sumoPosition.getY()));
